...
# ----------------------
# 指定保存图形的目录
output_dir <- "d:/JianMoXiaosai/higher_edu_plots"  # 修改为您希望保存图形的目录
...
df <- data.frame(
  ...
  y = c(599526,618273,632723,642263,643872,658327,700145,726957,755552,710245,715922), # 在校生人数
...
p_diffx22 <- ggplot(data.frame(time = df$time[-c(1, 2)], value = diff_x2_2), aes(x = time, y = value)) +
  geom_line(color = "purple", linewidth = 0.8) +
  labs(title = "x2两次差分时间序列图", x = "年份", y = "两次差分值") +
  theme_minimal()
...
forecast_result <- forecast(best_model, xreg = matrix(c(29, 208854, 29, 206101, 29, 181824), ncol = 2, byrow = TRUE), h = 3)
